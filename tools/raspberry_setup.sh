...
Options:
  ...
  -d, --droneATSM-id ID           Specify Drone ID (e.g., 1, 2) [Required]
...
parse_args() {
    ...
    PARSED_ARGS=$(getopt -o b:d:k:u:h --long branch:,droneATSM-id:,netbird-key:,management-url:,repo-url:,ssh-key-path:,skip-netbird,skip-mavsdk,skip-gpio,skip-sudoers,help -- "$@")
    if [[ $? -ne 0 ]]; then
        usage
        exit 1
    fi
...
setup_ssh_key_for_git() {
    ...
    if [[ -f "$SSH_KEY_PATH" ]]; then
        ...
        ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_PATH" -N "" -C "droneATSM$DRONE_ID@$(hostname)"
...
# =============================================================================
configure_hostname() {
    echo "Configuring hostname to 'droneATSM$DRONE_ID'..."
    echo "droneATSM$DRONE_ID" | sudo tee /etc/hostname > /dev/null
    sudo sed -i "s/.*127.0.1.1.*/127.0.1.1\tdroneATSM$DRONE_ID/" /etc/hosts

    echo "Reloading hostname service to apply changes immediately..."
    sudo hostnamectl set-hostname "droneATSM$DRONE_ID"
...
setup_netbird() {
    ...
    echo "Netbird reconnected with new hostname 'droneATSM$DRONE_ID'."
    unset NETBIRD_KEY
